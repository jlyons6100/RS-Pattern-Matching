...
def test_models_on_row(row, values):
    arr = list(row)
    global m,n, n_splits
    X = []
    Y = []
...
def test_models_on_cat(model_averages, headers, cat):
    ...
    for index, row in df.iterrows():
        test_models_on_row(row, values)
    # Printing categorical averages
    for model_name in values:
        #print(model_name)
        average = sum(values[model_name])/len(values[model_name])
        #print(average)
        if model_name not in model_averages: model_averages[model_name] = [average]
        else: model_averages[model_name].append(average)
...
def machine_learning():
    ...
    print("Model Averages for EVERY category: (" +str(m) + ","+str(n)+")")
